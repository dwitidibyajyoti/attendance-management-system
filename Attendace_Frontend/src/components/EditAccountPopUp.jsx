...
const EditAccountPopUp = ({setShowEditPopUp,userId}) => {
    ...
    return (
        <div className="flex items-center justify-center bg-gray-900 bg-opacity-50 fixed inset-0 z-50">
          <div className="bg-slate-50 p-4 rounded-2xl shadow-md h-fit w-[40%] relative">
            <div>
              <span className="absolute left-8 text-2xl opacity-90 font-bold mb-2">Update Details</span>
              <button onClick={()=>setShowEditPopUp(false)} className="absolute right-4 top-3 text-red-600 text-3xl"><FontAwesomeIcon icon={faTimes} /></button>
            </div><br /><br />
            {submitted ? (
              <p className='text-green-600'>Your details Updated successfully!</p>
            ) : (
              <form className='flex flex-col space-y-7 justify-center items-center' onSubmit={handleSubmit}>
                ...
                <div className='w-[90%] relative'>
                <button type='submit' className='border h-10 w-full text-white bg-[#212121] hover:text-blue-100 hover:bg-slate-800 rounded-xl tracking-wider'>Update</button>
                <div>
                </div>  
                </div>
